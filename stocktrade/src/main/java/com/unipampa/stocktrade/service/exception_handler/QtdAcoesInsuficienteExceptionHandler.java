...
@Component
public class QtdAcoesInsuficienteExceptionHandler extends ExceptionHandler {
    @Override
    public String handle(TipoException tipoException, HttpSession session, RegistroRepository registroRepository) {
        if (tipoException == TipoException.QTD_ACOES_INSUFICIENTE) {
            String erro = "Quantidade de ações disponíveis insuficiente!";
